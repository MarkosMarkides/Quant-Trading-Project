...
class PriceStreamer(StreamBase):
    ...
    def run(self):
        ...
        # stream=True creates an HTTP connection that stays open, continuously receiving data from the API
        resp = requests.get(url, params=params, headers=defs.SECURE_HEADER, stream=True)

        # The for loop, in this case, is just like doing: while True: ... 
        # It continuously fetches prices as long as the connection to the API remains active.
